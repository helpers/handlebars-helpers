...
exports.join = function(array, sep) {
  return array.join(utils.isUndefined(sep) ? ' ' : sep);
};
...
exports.filter = function(arr, val, options) {
  var res = [];
  var content = '';
  var data;
  ...
  if (options.hash && options.hash.property) {
    var search = {};
    search[options.hash.property] = val;
    res = _.filter(arr, search);

  } else {

    // filter on a string value
    res = _.filter(arr, function(v, k) {
      return val === v;
    });
  }
