...
var helpers = {
  ...
  /**
   * {{forEachRandom}}
   * Randomises the array and performs a forEach on it
   * @author: John Cashmore <https://github.com/JohnCashmore>
   *
   * @param  {[type]}   array [description]
   * @param  {Function} fn    [description]
   * @return {[type]}         [description]
   ...
   *   ];
   *
   *   {{#forEachRandom accounts}}
   *     <a href="mailto:{{ email }}" title="Send an email to {{ name }}">
   *       {{ name }}
   *     </a>{{#unless isLast}}, {{/unless}}
   *   {{/forEachRandom}}
   */
  forEachRandom: function (array, fn) {
    var _self = this;
    ...
    return this.forEach(shuffle(array), fn);
  },
  /**
   * {{eachProperty}}
