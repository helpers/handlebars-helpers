...
utils.falsey = value => {
  ...
  if (typeof value === 'string') {
    return keywords.includes(value) || keywords.includes(value.toLowerCase());
  }
...
utils.chunk = (arr, size) => Array(Math.ceil(arr.length / size)).fill().map((_, index) => index * size).map(begin => arr.slice(begin, begin + size));
