...
/**
 * Block helper that returns renders the block if the given
 * array has any elements. Optionally specify an inverse block
 * to render when the array does not have elements.
 *
 * ```handlebars
 * {{any "['a', 'b', 'c']"}}
 * //=> true
...
helpers.eachIndexPlusOne = function(array, options) {
  var result = '';
  for (var i = 0; i < array.length; i++) {
    result += options.fn({item: array[i], index: i + 1});
  }
  return result;
};
