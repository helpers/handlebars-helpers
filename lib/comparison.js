...
/**
 * Block helper that renders the block if **any** of the given patterns
 * (in the second argument) exist in the first value. If false, the
 * inverse block is rendered (if specified).
 *
 * @name .any
 * @param {type} `value`
 * @param {type} `pattern`
 * @param {type} `options` Handlebars supplied options object.
 * @return {String}
 * @block
 * @api public
 */

helpers.any = function(value, pattern, options) {
...
helpers.has = function(value, pattern, options) {
  if (typeof value === 'undefined' || typeof pattern === 'undefined') {
    return options.inverse(this);
  }
