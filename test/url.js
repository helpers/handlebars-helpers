'use strict';

var should = require('should');
var Handlebars = require('handlebars');
var helpers = require('..');

Handlebars.registerHelper(helpers('url'));

describe('{{urlResolve}}', function() {
  it('should take a base URL, and a href URL, and resolve them as a browser would', function() {
    var template = Handlebars.compile('{{urlResolve "/one/two/three" "four"}}');
    template().should.equal('/one/two/four');
  });
  it('should take a base URL, and a href URL, and resolve them as a browser would', function() {
    var template = Handlebars.compile('{{urlResolve "http://example.com/" "/one"}}');
    template().should.equal('http://example.com/one');
  });
  it('should take a base URL, and a href URL, and resolve them as a browser would', function() {
    var template = Handlebars.compile('{{urlResolve "http://example.com/one" "/two"}}');
    template().should.equal('http://example.com/two');
  });
});

describe('{{stripQuerystring}}', function() {
  it('should return a url without its query string.', function() {
    var template = Handlebars.compile('{{stripQuerystring "http://example.com?tests=true"}}');
    template().should.equal('http://example.com');
  });
});

describe('{{encodeURI}}', function() {
  it('should return an encoded uri string.', function() {
    var template = Handlebars.compile('{{encodeURI "http://example.com?comment=Thyme &time=again"}}');
    template().should.equal('http%3A%2F%2Fexample.com%3Fcomment%3DThyme%20%26time%3Dagain');
  });
});

describe('{{decodeURI}}', function() {
  it('should return an decoded uri string.', function() {
    var template = Handlebars.compile('{{decodeURI "http%3A%2F%2Fexample.com%3Fcomment%3DThyme%20%26time%3Dagain"}}');
    template().should.equal('http://example.com?comment=Thyme &amp;time=again');
  });
});

describe('{{urlParse}}', function() {
  it('should take a URL string, and return an object stringified to JSON.', function() {
    var template = Handlebars.compile('{{{urlParse "http://foo.com/bar/baz?key=value" "json"}}}');
    JSON.parse(template()).should.eql({
      "protocol": "http:",
      "slashes": true,
      "auth": null,
      "host": "foo.com",
      "port": null,
      "hostname": "foo.com",
      "hash": null,
      "search": "?key=value",
      "query": "key=value",
      "pathname": "/bar/baz",
      "path": "/bar/baz?key=value",
      "href": "http://foo.com/bar/baz?key=value"
    });
  });
});

